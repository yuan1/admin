var ConfigJS = function () {
    ...
    var init = function (){
        ...
        $('#tree-config').on('select_node.jstree', function (e, data) {
            ...
            $("#addbtn").show();
            $("#addbtn").attr('data-url', '/config/addItem.html?configId=' + configId);
            loadInput();
            submitItemFrom();
        });
        $("#addbtn").hide();
...
function submitItemFrom() {
    $("#js-item-form").ajaxSubmit({
        type: 'post', // 提交方式 get/post
        url: '/config/itemList.html?configId=' + configId,
        success: function (data) { // data 保存提交后返回的数据，一般为 json 数据
            // 此处可对 data 作相关处理
